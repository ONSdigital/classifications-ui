...
from cims_ui import app # get_classifications_cached
from cims_ui.page_controllers.b_multiple_entries.utils.multiple_entries_utils import job_url_if_authorised
from cims_ui.page_error import page_error
...
def download_handler(file_name):
  ...
  elif file_name == 'example_multiple_entries':
    f = open(
        f'{root_dir_path}/static/downloads/example_multiple_entries_upload.csv',
        'rb')
  elif file_name == 'example_multiple_entries_big':
    f = open(
        f'{root_dir_path}/static/downloads/example_multiple_entries_5k_upload.csv',
        'rb')
  elif file_name == 'tool_tip_clerical_information':
    f = open(
        f'{root_dir_path}/static/downloads/tool_tip_clerical_information.csv',
        'rb')
  elif file_name == 'uprn_example_multiple_entries':
    f = open(
        f'{root_dir_path}/static/downloads/uprn_example_multiple_entries_upload.csv',
        'rb')
